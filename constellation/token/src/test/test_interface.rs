...
use crate::{contract::ConstellationTokenClient, storage::admin};
...
type TokenClient<'T> = token::Client<'T>; 
...
pub(crate) fn initialize_token<'a>(
  ...
  //  tokens: (Address, Address, Address),
) -> (ConstellationTokenClient<'a>, Address, Address, (TokenClient<'a>,TokenClient<'a>,TokenClient<'a> )) {
    ...
    (ct, admin, manager, (token1, token2, token3))
}
...
fn test() {
    ...
    let (mut ct, admin1, manager, tokens) = initialize_token(
        &e,
        create_constellation_token(&e), 
    );

    let (token1, token2, token3) = tokens;
    token1.mint(&admin1, &1000i128);
    token2.mint(&admin1, &1000i128);
    token3.mint(&admin1, &1000i128);

    token1.approve(&admin1, &ct.address, &1000i128, &1000u32);
    token2.approve(&admin1, &ct.address, &1000i128, &1000u32);
    token3.approve(&admin1, &ct.address, &1000i128, &1000u32);
        ...
        std::vec![(
            ...
            AuthorizedInvocation {
                function: AuthorizedFunction::Contract((
                    ct.address.clone(),
                    symbol_short!("mint"),
                    (&admin1, 1000_i128).into_val(&e),
                )),
                sub_invocations: std::vec![]
            }
        )]
    );
    // assert_eq!(ct.balance(&admin), 1000);
